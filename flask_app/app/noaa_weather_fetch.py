...
from flask_app.app.utils import log_error_and_continue, calculate_distance_nm
from flask_app.app.fetch_NOAA_offshore_buoys import fetch_offshore_buoys
from flask_app.app.noaa_weather_alerts_fetch import fetch_weather_alerts_zone
...
def fetch_nearest_weather_stations(lat, lon, position_label='A'):
    ...
    try:
        ...
        if is_maritime:
            logging.info(f"🌊 Position {position_label} is maritime (within 5NM of shore). Fetching offshore buoys.")
        ...
        if combined_stations.empty:
            logging.warning(f"⚠️ No weather stations found for Position {position_label}")
        ...
        if complete_stations.empty:
            logging.warning(f"⚠️ No complete weather stations found for Position {position_label}")
        ...
        if weather_alerts_zone:
            logging.info(f"🌐 Weather zone for Position {position_label}: {weather_alerts_zone}")
        ...
        log_error_and_continue(f"❌ Error fetching weather stations for Position {position_label}: {e}")
        return pd.DataFrame()
...
def _fetch_onshore_weather_stations(lat, lon, position_label):
    ...
    """
    log_error_and_continue(f"❌ ❌ Error fetching onshore stations: Onshore station fetcher is assumed implemented elsewhere.")
    return pd.DataFrame()
